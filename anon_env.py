...
class Intersection:
    ...
    def log_one_vehicle(self,vehicle,path_to_log):
        ...
        df = [vehicle,self.dic_vehicle_arrive_leave_time[vehicle]["enter_time"],self.dic_vehicle_arrive_leave_time[vehicle]["leave_time"]]
        df = pd.DataFrame(df)
        df = df.transpose()
    ...
    def update_neighbor_info(self, neighbors, dic_feature):
        ...
        for i in range(len(neighbors)):
            ...
            if neighbor is None:
                ...
                example_dic_feature["lane_num_vehicle_downstream_{0}".format(i)] = none_dic_feature["lane_num_vehicle_downstream"]
            else:
                ...
                example_dic_feature["lane_num_vehicle_downstream_{0}".format(i)] = neighbor.dic_feature["lane_num_vehicle_downstream"]
    ...
    def _update_feature_map(self, simulator_state,test_flag):
        ...
        dic_feature["delta_lane_num_vehicle"] = [dic_feature["lane_num_vehicle"][i] - dic_feature["lane_num_vehicle_downstream"][i] for i in range(len(dic_feature["lane_num_vehicle_downstream"]))]
...
class AnonEnv:
    ...
    def reset(self):
        ...
        self.system_states["num_total_veh"] = np.sum([len(self.system_states["get_lane_vehicles"][lane]) for lane in self.list_lanes])
